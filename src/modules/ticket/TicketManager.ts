...
import { TicketModel, ITicket } from '../../database/models/Ticket';
...
export class TicketManager {
  static async openTicket(guild: Guild, member: GuildMember, type: string): Promise<string | null> {
    ...
    const threadName = config.threadNameTemplate
      .replace('{type}', type)
      .replace('{username}', member.user.username)
      .replace('{id}', String(ticketNumber));

    const panelChannel = config.panelChannelId
      ? guild.channels.cache.get(config.panelChannelId) as TextChannel | undefined
      : null;
    ...
    const now = new Date();
    const openMsg = config.openMessageTemplate
